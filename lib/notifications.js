...
const sanitizeEvent = (event, getUserById) => {
  ...
  if (!safe.ownerName && typeof event.ownerName === 'string') {
    const trimmed = event.ownerName.trim();
    if (trimmed) safe.ownerName = trimmed;
  }

  if (!safe.ownerName && safe.ownerId && typeof getUserById === 'function') {
...
const createNotificationsManager = ({ getEvents, getUserById, logger, config = {} }) => {
  ...
  const evaluate = async (referenceDateInput = new Date()) => {
    ...
    if (typeof getEvents === 'function') {
      try {
        events = await Promise.resolve(getEvents());
      } catch (err) {
        log.error('[notifications] Falha ao carregar eventos para notificação:', err);
        events = [];
      }
    }
  ...
  const sendNotifications = async ({
    ...
    const evaluation = await evaluate(referenceDate);
    const response = {
      reason,
  ...
  return {
    evaluate,
    sendNotifications,
    buildMessage: async (referenceDate) => buildMessage(await evaluate(referenceDate)),
